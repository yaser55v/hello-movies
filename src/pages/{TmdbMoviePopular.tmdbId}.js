...
const MoviePopularTemplate = ({ data }) => {
  ...
  useEffect(() => {
    const url = `${movieUrl}${tmdbId}?api_key=${API_KEY}&language=en-US&region=US&append_to_response=videos,similar,credits,release_dates`
...
export const query = graphql`
  query getOneMovie($tmdbId: String) {
    tmdbMoviePopular(tmdbId: { eq: $tmdbId }) {
      tmdbId
    }
  }
`
export default MoviePopularTemplate
